...
class Main:
    ...
    def create_gui(self):
        ...
        tk.Button(credentials_frame_body, text='Browse', bg=button_color, command=on_browse).pack(side='left',
                                                                                                  padx=padx)
        ...
        button_width = 17
        button_height = 2
        tk.Button(buttons_frame, text='Connect', width=button_width, height=button_height, bg=button_color,
                  command=self.on_connect).pack(side='left', padx=padx)
        tk.Button(buttons_frame, text='Connect to emulator', width=button_width, height=button_height,
                  bg=button_color, command=self.on_emulator_connect).pack(side='left', padx=padx)
        tk.Button(buttons_frame, text='Exit', width=button_width, height=button_height, bg=button_color,
                  command=self.on_close).pack(side='left', padx=padx)
    ...
    def on_connect(self):
        ...
        if not credentials_path:
            messagebox.showerror('Error', 'Couldn\'t find the Google credetials. Please add the path to your Google '
                                          'credentials file.')
            return
